...
힙 내의 객체 중 참조되지 않는 객체를 가비리자고 한다.  
가비지를 찾아내고 찾아낸 가비지극 처리해서 힙의 메모리를 회수하는 것을 GC라고 한다.

## Minor GC

- 새로 생성된 대부분의 객체는 Eden 영역에 위치한다.
- Eden 영역이 꽉 차게 되면 GC가 발생한다.
- GC 발생 후 살아남은 객체는 Survivor 영역중 하나로 이동한다.
- 이 과정을 반복하며 살아남은 객체는 Old 영역으로 이동한다.

## Major GC

- Old 영역에 있는 모든 객체들을 검사하여 참조되고 있지 않은 객체들을 한번에 삭제한다.
- 이때 GC를 수행하는 스레드를 제회한 모든 프로세스가 정지되는데 이를 **'Stop The World' (STW)**라고 칭한다.
