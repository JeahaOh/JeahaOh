...
- 물류 시스템 모듈화
- 특정 택배사에 종속적인 배송 시스템을 모듈화하여 타 택배사 이용 가능하도록 개편
- 상품별 출고지 (자사물류센터/CJ 풀필먼트) 구분 기능 및 3자 물류 기능 개발

#### 이벤트 / 프로모션 기능 개선

- 쿠폰 다운로드, 사다리타기, 응모, 랜덤 뽑기, 추첨 등의 마케팅 기능 컨텐츠가 한번에 한개만 생성 가능 하던 것을 페이지 안에서 여러 컨텐츠를 포함 가능하도록 구조 개선
- 페이징 처리
  - 마켓팅 LMS 발송 후 동접 500명만 넘어도 페이지 로딩하는데 1분 걸리던 이벤트 페이지를 페이징 처리하여 3초대에 렌더링까지 끝나도록 개선
